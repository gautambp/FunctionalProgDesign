...
object TweetLength {
  ...
  def tweetRemainingCharsCount(tweetText: Signal[String]): Signal[Int] = {
    new Signal[Int](140 - tweetLength(tweetText.apply()))
  }

  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] = {
    new Signal[String]( if (remainingCharsCount.apply() >= 15) "green" else if (remainingCharsCount.apply() >= 0) "orange" else "red" )  
  }
